...
bool FindPubKeyPos(std::string& pubKeyIn, CDiskPubKeyPos& pubKeyPos) {
    ...
    while (pBlockIndex && pindexBest) {
        if (pindexBest->nHeight - pBlockIndex->nHeight + 1 <
            COINBASE_MATURITY + 20) {
            printf("%s: block not maturity, height:%u\n", __func__,
                   pindexBest->nHeight - pBlockIndex->nHeight + 1);
            return false;
        }
        FILE* file = OpenBlockFile(pBlockIndex->GetBlockPos(), true);
        if (NULL == file)
            return error("%s() : open file blk%d.dat at %u error",
                         __PRETTY_FUNCTION__, pBlockIndex->nFile,
        ...
        try {
            ...
            return error(
                "%s: Deserialize or I/O error - %s, file blk%d.dat at %u",
                __func__, e.what(), pBlockIndex->nFile, pBlockIndex->nDataPos);
        }
        ...
        for (unsigned int i = 0; i < nTxCount; i++) {
            ...
            for (unsigned int i = 0; i < nVinSize; i++) {
                ...
                if (found != std::string::npos) {
                    ...
                    offsetVin += sizeof(COutPoint);  // transation hash length +
                                                     // prevout index length
                    offsetVin += GetSizeOfCompactSize(nScriptSigSize);
                    pubKeyPos.nPubKeyOffset =
                        offset + offsetVin +
                        found / 2;  // the offset in the current block, 2 hex
                                    // char for one byte
                    unsigned int npubKeyLength =
                        GetSizeOfCompactSize(pubKeyIn.length());
                    printf(
                        "public key found: offset:%u, offsetVin:%u, found:%u, "
                        "npubKeyLength:%u. \n",
                    ...
                    offsetVin +=
                        ::GetSerializeSize(txIn, SER_DISK, CLIENT_VERSION);
                }
            }

            offset += GetSerializeSize(tx, SER_DISK,
                                       CLIENT_VERSION);  // CTransaction length
        }
...
bool GetPubKeyByPos(CDiskPubKeyPos pos, CPubKey& pubKey) {
    ...
    if (!bFindBlockByHeight)
        return error("%s() : can't find block at height: %u",
                     __PRETTY_FUNCTION__, pos.nHeight);

    // in scripts, the public key is deserialize as
    // 4e (21 52 02 00) (e2 b8 8a 76 1b 0d d7 8e b3...)--4e is the opcode, (21
    // 52 02 00) is the length
    CDiskBlockPos blockPos(pblockindex->nFile,
                           pblockindex->nDataPos + pos.nPubKeyOffset);
    FILE* pFile = OpenBlockFile(blockPos, true);
    if (NULL == pFile)
        return error("%s() : open file blk%d.dat error", __PRETTY_FUNCTION__,
                     pblockindex->nFile);
    ...
    try {
        ...
        if (opcode < OP_PUSHDATA1) {
            ...
            return error("%s() : invalid opcode=%x or I/O error",
                         __PRETTY_FUNCTION__, opcode);

        // currently rainbow public key size is fixed, maybe change in future,
        // change this
        if (nSize != RAINBOW_PUBLIC_KEY_SIZE)
            return error("%s() : public key size %d invalid",
                         __PRETTY_FUNCTION__, nSize);

        unsigned int i = 0;
        while (i < nSize) {
            unsigned int blk = std::min(
                nSize - i, (unsigned int)(1 + 4999999 / sizeof(unsigned char)));
...
bool UpdatePubKeyPos(CPubKey& pubKey, const std::string& address) {
    ...
    if (!pwalletMain->GetPubKeyPos(address, pos)) {
        ...
        // read the public key by position and compare, if not match, find it
        // again
        CPubKey diskPubKey;
        ...
        if (!GetPubKeyByPos(pos, diskPubKey)) {
            printf(
                "can't get public key %s at height=%u, offset=%u. will find "
                "again! \n",
                address.c_str(), pos.nHeight, pos.nPubKeyOffset);
            pos.SetNull();
        ...
        if (pos.IsNull()) {
            ...
            if (!FindPubKeyPos(strPubKey, pos)) {
                ...
                printf(
                    "find again, public key %s found at height=%d, offset=%u. "
                    "\n",
                    address.c_str(), pos.nHeight, pos.nPubKeyOffset);
                pwalletMain->AddPubKeyPos(address, pos);
                found = true;
...
void PubKeyScanner(CWallet* pwalletMain) {
    ...
    try {
        while (true) {
            ...
            for (std::set<CKeyID>::iterator it = setAddress.begin();
                 it != setAddress.end(); ++it) {
                ...
                if (!UpdatePubKeyPos(pubKey, address)) {
                    printf(
                        "address %s update public key position return false\n",
                        address.c_str());
                    allPosFound = false;
                    continue;
                } else {
                    std::map<CTxDestination, std::string>::iterator mi =
                        pwalletMain->mapAddressBook.find(*it);
                    if (mi != pwalletMain->mapAddressBook.end())
                        pwalletMain->NotifyAddressBookChanged(
                            pwalletMain, *it, mi->second, true, CT_UPDATED);
                }
            }
