...
Value setgenerate(const Array& params, bool fHelp) {
    ...
    GenerateAbcmints(fGenerate, pwalletMain);
    return Value::null;
}
...
Value gethashespersec(const Array& params, bool fHelp) {
    ...
    if (GetTimeMillis() - nHPSTimerStart > 8000) return (boost::int64_t)0;
...
Value getwork(const Array& params, bool fHelp) {
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getwork [data]\n"
            "If [data] is not specified, returns formatted hash data to work "
            "on:\n"
            "  \"midstate\" : precomputed hash state after hashing the first "
            "half of the data (DEPRECATED)\n"  // deprecated
            "  \"data\" : block data\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                           "Abcmint is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Abcmint is downloading blocks...");
    ...
    if (params.size() == 0) {
        ...
        if (pindexPrev != pindexBest ||
            ...
            if (pindexPrev != pindexBest) {
                ...
                mapNewBlock.clear();
                BOOST_FOREACH (CBlockTemplate* pblocktemplate,
                               vNewBlockTemplate)
                    delete pblocktemplate;
                vNewBlockTemplate.clear();
            }

            // Clear pindexPrev so future getworks make a new block, despite any
        ...
        // Save
        mapNewBlock[pblock->hashMerkleRoot] =
            make_pair(pblock, pblock->vtx[0].vin[0].scriptSig);
        ...
        // Get saved block
        if (!mapNewBlock.count(pdata->hashMerkleRoot)) return false;
...
Value getblocktemplate(const Array& params, bool fHelp) {
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "  \"transactions\" : contents of non-coinbase transactions that "
            "should be included in the next block\n"
            "  \"coinbasevalue\" : maximum allowable input to coinbase "
            "transaction, including the generation award and transaction fees\n"
            "  \"target\" : hash target\n"
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx) {
        ...
        entry.push_back(
            Pair("sigops", pblocktemplate->vTxSigOps[index_in_template]));
...
Value submitblock(const Array& params, bool fHelp) {
    ...
    if (!fAccepted) return "rejected";  // TODO: report validation state
