...
bool CCryptoKeyStore::AddKey(const CKey &key) {
    {
        LOCK(cs_KeyStore);
        if (!IsCrypted()) return CBasicKeyStore::AddKey(key);

        if (IsLocked()) return false;

        std::vector<unsigned char> vchCryptedSecret;
        CPubKey vchPubKey = key.GetPubKey();
        if (!EncryptSecret(vMasterKey, key.GetPrivKey(), vchPubKey.GetHash(),
                           vchCryptedSecret))
            return false;

        if (!AddCryptedKey(key.GetPubKey(), vchCryptedSecret)) return false;
    }
    return true;
}
...
bool CCryptoKeyStore::GetKey(const CKeyID &address, CKey &keyOut) const {
    {
        ...
        if (mi != mapCryptedKeys.end()) {
            ...
            const std::vector<unsigned char> &vchCryptedSecret =
                (*mi).second.second;
            CSecret vchSecret;
            if (!DecryptSecret(vMasterKey, vchCryptedSecret,
                               vchPubKey.GetHash(), vchSecret))
                return false;
            if (vchSecret.size() != RAINBOW_PRIVATE_KEY_SIZE) return false;
...
bool CCryptoKeyStore::EncryptKeys(CKeyingMaterial &vMasterKeyIn) {
    {
        ...
        BOOST_FOREACH (KeyMap::value_type &mKey, mapKeys) {
            CKey key;
            if (!key.SetPrivKey(mKey.second.GetPrivKey())) return false;
            const CPubKey vchPubKey = mKey.second.pubKey;
            std::vector<unsigned char> vchCryptedSecret;
            if (!EncryptSecret(vMasterKeyIn, key.GetPrivKey(),
