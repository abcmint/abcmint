...
Value listunspent(const Array& params, bool fHelp) {
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "Optionally filtered to only include txouts paid to specified "
            "addresses.\n"
        ...
        BOOST_FOREACH (Value& input, inputs) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(
                    ...
                    string("Invalid Abcmint address: ") + input.get_str());
            if (setAddress.count(address))
                throw JSONRPCError(
                    RPC_INVALID_PARAMETER,
                    string("Invalid parameter, duplicated address: ") +
                        input.get_str());
    ...
    BOOST_FOREACH (const COutput& out, vecOutputs) {
        ...
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)) {
            entry.push_back(
                Pair("address", CAbcmintAddress(address).ToString()));
...
Value signrawtransaction(const Array& params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 4)
        throw runtime_error(
            ...
            "[{\"txid\":txid,\"vout\":n,\"scriptPubKey\":hex,\"redeemScript\":"
            "hex},...] [<privatekey1>,...] [sighashtype=\"ALL\"]\n"
            ...
            "chain.\n"
            "Third optional argument (may be null) is an array of "
            "base58-encoded private\n"
            "keys that, if given, will be the only keys used to sign the "
            "transaction.\n"
            "Fourth optional argument is a string that is one of six values; "
            "ALL, NONE, SINGLE or\n"
            "ALL|ANYONECANPAY, NONE|ANYONECANPAY, SINGLE|ANYONECANPAY.\n"
            "Returns json object with keys:\n"
            "  hex : raw transaction with signature(s) (hex-encoded string)\n"
            "  complete : 1 if transaction has a complete set of signature (0 "
            "if not)" +
            HelpRequiringPassphrase());

    RPCTypeCheck(params, list_of(str_type)(array_type)(array_type)(str_type),
                 true);
    ...
    CCoinsViewCache view(viewDummy);
    {
        ...
        view.SetBackend(viewMempool);  // temporarily switch cache backend to
                                       // db+mempool view

        BOOST_FOREACH (const CTxIn& txin, mergedTx.vin) {
            const uint256& prevHash = txin.prevout.hash;
            CCoins coins;
            view.GetCoins(prevHash,
                          coins);  // this is certainly allowed to fail
        }
    ...
    if (params.size() > 1 && params[1].type() != null_type) {
        ...
        BOOST_FOREACH (Value& p, prevTxs) {
            ...
            RPCTypeCheck(prevOut,
                         map_list_of("txid", str_type)("vout", int_type)(
            ...
            if (fGivenKeys && scriptPubKey.IsPayToScriptHash()) {
                RPCTypeCheck(
                    prevOut,
                    map_list_of("txid", str_type)("vout", int_type)(
                        "scriptPubKey", str_type)("redeemScript", str_type));
    ...
    if (params.size() > 3 && params[3].type() != null_type) {
        static map<string, int> mapSigHashValues =
            boost::assign::map_list_of(string("ALL"), int(SIGHASH_ALL))(
                ...
                int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))(string("NONE"),
                                                         int(SIGHASH_NONE))(
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (!view.GetCoins(txin.prevout.hash, coins) ||
            !coins.IsAvailable(txin.prevout.n)) {
            fComplete = false;
            continue;
        }
        ...
        BOOST_FOREACH (const CTransaction& txv, txVariants) {
            txin.scriptSig = CombineSignatures(
                prevPubKey, mergedTx, i, txin.scriptSig, txv.vin[i].scriptSig);
        }
        ...
        if (!fHave) {
            ...
            if (!tx.AcceptToMemoryPool(state, true, false))
                throw JSONRPCError(
                    ...
                    "TX rejected");  // TODO: report validation state
        }
    }
    if (fHave) {
