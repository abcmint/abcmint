...
static const unsigned char g_internal_prefix[] = {0xFD, 0x96, 0xA1,
                                                  0xB6, 0x77, 0x5E};
...
bool ConnectSocketByName(CService& addr, SOCKET& hSocketRet,
    ...
    CService addrResolved(CNetAddr(strDest, fNameLookup && !nameproxy.second),
                          port);
    ...
    if (!nameproxy.second) return false;
        ...
        default:
            switch (ourNet) {
                ...
                case NET_IPV4:
                    return REACH_IPV4;
                case NET_TOR:
                    return REACH_PRIVATE;  // either from Tor, or don't care
                                           // about our address
            }
    }
}
