...
void CDBEnv::EnvShutdown() {
    ...
    if (!fMockDb) DbEnv((uint32_t)0).remove(path.string().c_str(), 0);
}
...
bool CDBEnv::Salvage(std::string strFile, bool fAggressive,
    ...
    if (result == DB_VERIFY_BAD) {
        ...
        if (!fAggressive) {
            printf(
                "Error: Rerun with aggressive mode to ignore errors and "
                "continue.\n");
            return false;
        }
    }
...
CDB::CDB(const char* pszFile, const char* pszMode)
    ...
    {
        LOCK(bitdb.cs_db);
        if (!bitdb.Open(GetDataDir())) throw runtime_error("env open failed");
        ...
        if (pdb == NULL) {
            ...
            if (fMockDb) {
                ...
                if (ret != 0)
                    throw runtime_error(
                        strprintf("CDB() : failed to configure for no temp "
                                  "file backing for database %s",
                                  pszFile));
            }
...
void CDB::Flush() {
    ...
    bitdb.dbenv.txn_checkpoint(nMinutes ? GetArg("-dblogsize", 100) * 1024 : 0,
                               nMinutes, 0);
}
    ...
    while (true) {
        {
            ...
            if (!bitdb.mapFileUseCount.count(strFile) ||
                ...
                {  // surround usage of db with extra {}
                    ...
                    if (pcursor)
                        while (fSuccess) {
                            ...
                            int ret2 = pdbCopy->put(NULL, &datKey, &datValue,
                                                    DB_NOOVERWRITE);
                ...
                if (fSuccess) {
                    ...
                    if (dbB.rename(strFileRes.c_str(), NULL, strFile.c_str(),
                                   0))
                        fSuccess = false;
                }
                if (!fSuccess)
    ...
    if (!fDbEnvInit) return;
    {
        ...
        if (fShutdown) {
            ...
            if (mapFileUseCount.empty()) {
                ...
                Close();
                if (!fMockDb) boost::filesystem::remove_all(path / "database");
            }
        }
    }
}
...
bool CAddrDB::Write(const CAddrMan& addr) {
    ...
    if (!fileout) return error("CAddrman::Write() : open failed");
...
bool CAddrDB::Read(CAddrMan& addr) {
    ...
    if (!filein) return error("CAddrman::Read() : open failed");
