...
Value getpeerinfo(const Array& params, bool fHelp) {
    ...
    BOOST_FOREACH (const CNodeStats& stats, vstats) {
        ...
        obj.push_back(
            Pair("services", strprintf("%08" PRI64x, stats.nServices)));
        ...
        // from corrupting or modifiying the JSON output by putting special
...
Value addnode(const Array& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 2 ||
        ...
        throw runtime_error(
            ...
            "Attempts add or remove <node> from the addnode list or try a "
...
Value getaddednodeinfo(const Array& params, bool fHelp) {
    ...
    LOCK(cs_vNodes);
    for (list<pair<string, vector<CService> > >::iterator it =
             laddedAddreses.begin();
        ...
        BOOST_FOREACH (CService& addrNode, it->second) {
            ...
            BOOST_FOREACH (CNode* pnode, vNodes)
                if (pnode->addr == addrNode) {
                    ...
                    fConnected = true;
                    node.push_back(Pair(
                        "connected", pnode->fInbound ? "inbound" : "outbound"));
                    break;
                }
