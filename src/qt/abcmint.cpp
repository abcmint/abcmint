...
int main(int argc, char *argv[])
{
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        ...
        // (which not yet possible because lang=XX can be overridden in abcmint.conf in the data directory)
    ...
    // Load e.g. abcmint_de.qm (shortcut "de" needs to be defined in abcmint.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        app.installTranslator(&translatorBase);

    // Load e.g. abcmint_de_DE.qm (shortcut "de_DE" needs to be defined in abcmint.qrc)
