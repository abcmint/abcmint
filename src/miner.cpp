...
#ifdef USE_GPU
static uint256 GPUMinerSearchSolution(uint256 hash, unsigned int nBits, int nBlockVersion, uint256 randomNonce, CBlockIndex* pindexPrev, int deviceID, int deviceCount) {
    unsigned int mEquations = nBits;
    unsigned int nUnknowns = nBits + 8;
    unsigned int nTerms = 1 + (nUnknowns+1)*(nUnknowns)/2;
    std::vector<uint8_t> coeffMatrix;
    coeffMatrix.resize(mEquations*nTerms);
    if (nBlockVersion == 1 || pindexPrev->nHeight < 25616) {
...
#endif
uint256 SerchSolution(uint256 hash, unsigned int nBits, int nBlockVersion, uint256 randomNonce, CBlockIndex* pindexPrev, int deviceID, int deviceCount) {
    unsigned int mEquations = nBits;
    unsigned int nUnknowns = nBits + 8;
    unsigned int nTerms = 1 + (nUnknowns+1)*(nUnknowns)/2;
    std::vector<uint8_t> coeffMatrix;
    ...
    uint256 nonce = 0;
#ifdef USE_GPU
    nonce = GPUMinerSearchSolution(hash, nBits, nBlockVersion, randomNonce, pindexPrev, deviceID, deviceCount);
#else
...
bool CheckSolution(uint256 hash, unsigned int nBits, uint256 prevblockhash, int nblockversion, uint256 nNonce) {
    ...
    if (nblockversion == 1 || height < 25217) {
        GenCoeffMatrix(hash, nBits, coeffMatrix);
    } else {
        NewGenCoeffMatrix(hash, nBits, coeffMatrix);
    }
...
void static AbcmintMiner(CWallet *pwallet, int deviceID, int deviceCount)
{
 ...
 //   LOCK(cs_setDevice);
    ...
    try { while(true) {
        ...
        while(true)
        {
            ...
            nNonceFound = SerchSolution(seedHash, pblock->nBits, pblock->nVersion,randomNonce,pindexPrev, deviceID, deviceCount);
...
void GenerateAbcmints(bool fGenerate, CWallet* pwallet)
{
 ...
 //   if (deviceCount < 2) {
        printf("The number of devices is %d, no more than 2 device with compute", deviceCount);
 //   }
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(boost::bind(&AbcmintMiner, pwallet, i, nThreads));
        MilliSleep(10000);
    }
    minerThreads->interrupt_all();
