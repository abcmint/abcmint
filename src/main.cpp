...
uint256 hashGenesisBlock(
    "0xcea89aa6adb81572f8b9e5f9b5d0184cbbc25208164cb1547decf3655da9dc77");
...
set<CBlockIndex *, CBlockIndexWorkComparator>
    setBlockIndexValid;  // may contain all CBlockIndex*'s that have validness
                         // >=BLOCK_VALID_TRANSACTIONS, and must contain those
                         // who aren't failed
...
std::map<uint256, CCoins>::iterator CCoinsViewCache::FetchCoins(
    ...
    if (it != cacheCoins.end() && it->first == txid) return it;
    CCoins tmp;
    if (!base->GetCoins(txid, tmp)) return cacheCoins.end();
    std::map<uint256, CCoins>::iterator ret =
        cacheCoins.insert(it, std::make_pair(txid, CCoins()));
    tmp.swap(ret->second);
    return ret;
}
...
CBlockIndex *CCoinsViewCache::GetBestBlock() {
    if (pindexTip == NULL) pindexTip = base->GetBestBlock();
    return pindexTip;
}
...
unsigned int CCoinsViewCache::GetCacheSize() { return cacheCoins.size(); }
...
bool CCoinsViewMemPool::GetCoins(const uint256 &txid, CCoins &coins) {
    if (base->GetCoins(txid, coins)) return true;
...
void EraseOrphanTx(uint256 hash) {
    if (!mapOrphanTransactions.count(hash)) return;
...
bool CTxOut::IsDust() const {
    ...
    return ((nValue * 1000) / (3 * ((int)GetSerializeSize(SER_DISK, 0) + 148)) <
            CTransaction::nMinRelayTxFee);
}
...
bool CTransaction::IsStandard(string &strReason) const {
    ...
    unsigned int sz =
        this->GetSerializeSize(SER_NETWORK, CTransaction::CURRENT_VERSION);
...
bool CTransaction::AreInputsStandard(CCoinsViewCache &mapInputs) const {
    if (IsCoinBase()) return true;  // Coinbases don't use vin normally
    ...
    for (unsigned int i = 0; i < vin.size(); i++) {
        ...
        if (whichType == TX_SCRIPTHASH) {
            ...
            if (!Solver(subscript, whichType2, vSolutions2)) return false;
            if (whichType2 == TX_SCRIPTHASH) return false;
...
int CMerkleTx::SetMerkleBranch(const CBlock *pblock) {
    ...
    if (pblock == NULL) {
        ...
        if (pcoinsTip->GetCoins(GetHash(), coins)) {
            ...
            if (pindex) {
                if (!blockTmp.ReadFromDisk(pindex)) return 0;
                pblock = &blockTmp;
            }
        }
    }
...
bool CTransaction::CheckTransaction(CValidationState &state) const {
    ...
    if (vout.empty())
        return state.DoS(
            10, error("CTransaction::CheckTransaction() : vout empty"));
    // Size limits
    ...
    BOOST_FOREACH (const CTxOut &txout, vout) {
        if (txout.nValue < 0)
            return state.DoS(100, error("CTransaction::CheckTransaction() : "
    ...
    BOOST_FOREACH (const CTxIn &txin, vin) {
        if (vInOutPoints.count(txin.prevout))
            return state.DoS(
                100,
                error("CTransaction::CheckTransaction() : duplicate inputs"));
...
int64 CTransaction::GetMinFee(unsigned int nBlockSize, bool fAllowFree,
    ...
    if (fAllowFree) {
        ...
        // * If we are relaying we allow transactions up to
        // DEFAULT_BLOCK_PRIORITY_SIZE - 1000
        //   to be considered to fall into this category. We don't want to
        //   encourage sending multiple transactions instead of one big
        //   transaction to avoid fees.
        // * If we are creating a transaction we allow transactions up to 1,000
        // bytes
        //   to be considered safe and assume they can likely make it into this
        //   section.
        if (nBytes <
            (mode == GMF_SEND ? 1000 : (DEFAULT_BLOCK_PRIORITY_SIZE - 1000)))
            nMinFee = 0;
    }

    // This code can be removed after enough miners have upgraded to version
    // 0.9. Until then, be safe when sending and require a fee if any output is
    // less than CENT:
    if (nMinFee < nBaseFee && mode == GMF_SEND) {
        BOOST_FOREACH (const CTxOut &txout, vout)
            if (txout.nValue < CENT) nMinFee = nBaseFee;
    }
...
void CTxMemPool::pruneSpent(const uint256 &hashTx, CCoins &coins) {
    ...
    std::map<COutPoint, CInPoint>::iterator it =
        mapNextTx.lower_bound(COutPoint(hashTx, 0));
...
bool CTxMemPool::accept(CValidationState &state, CTransaction &tx,
                        bool fCheckInputs, bool fLimitFree,
    ...
    if (tx.IsCoinBase())
        return state.DoS(
            100, error("CTxMemPool::accept() : coinbase as individual tx"));
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (mapNextTx.count(outpoint)) {
            ...
            if (!tx.IsNewerThan(*ptxOld)) return false;
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                COutPoint outpoint = tx.vin[i].prevout;
                if (!mapNextTx.count(outpoint) ||
    ...
    if (fCheckInputs) {
...

        {
            ...
            if (view.HaveCoins(hash)) return false;

            // do all inputs exist?
            // Note that this does not check for the presence of actual outputs
            // (see the next check for that), only helps filling in
            // pfMissingInputs (to determine missing vs spent).
            BOOST_FOREACH (const CTxIn txin, tx.vin) {
        ...
        if (fLimitFree && nFees < txMinFee)
            return error("CTxMemPool::accept() : not enough fees %s, %" PRI64d
                         " < %" PRI64d,
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee) {
            ...
            if (dFreeCount >= GetArg("-limitfreerelay", 15) * 10 * 1000)
                return error(
                    "CTxMemPool::accept() : free transaction rejected by rate "
                    "limiter");
            if (fDebug)
    ...
    ///// are we sure this is ok when loading transactions or restoring block
    ///txes
...
bool CTransaction::AcceptToMemoryPool(CValidationState &state,
                                      ...
                                      bool *pfMissingInputs) {
    try {
        return mempool.accept(state, *this, fCheckInputs, fLimitFree,
                              pfMissingInputs);
    ...
    // Remove transaction from memory pool
    {
        ...
        if (fRecursive) {
            for (unsigned int i = 0; i < tx.vout.size(); i++) {
                ...
                if (it != mapNextTx.end()) remove(*it->second.ptx, true);
            }
        }
        if (mapTx.count(hash)) {
            BOOST_FOREACH (const CTxIn &txin, tx.vin)
...
bool CTxMemPool::removeConflicts(const CTransaction &tx) {
    ...
    BOOST_FOREACH (const CTxIn &txin, tx.vin) {
        std::map<COutPoint, CInPoint>::iterator it =
            mapNextTx.find(txin.prevout);
        if (it != mapNextTx.end()) {
            const CTransaction &txConflict = *it->second.ptx;
            if (txConflict != tx) remove(txConflict, true);
        }
    }
    return true;
}
...
// Return transaction in tx, and if it was found inside a block, its hash is
// placed in hashBlock
    ...
    CBlockIndex *pindexSlow = NULL;
    {
        ...
        if (fAllowSlow) {  // use coin database to locate block that contains
            ...
            int nHeight = -1;
            {
                ...
                CCoins coins;
                if (view.GetCoins(hash, coins)) nHeight = coins.nHeight;
            }
            if (nHeight > 0) pindexSlow = FindBlockByHeight(nHeight);
        }
    }
...
CBlockIndex *FindBlockByHeight(int nHeight) {
    ...
    while (pblockindex->nHeight > nHeight) pblockindex = pblockindex->pprev;
    while (pblockindex->nHeight < nHeight) pblockindex = pblockindex->pnext;
    pblockindexFBBHLast = pblockindex;
    return pblockindex;
}

bool CBlock::ReadFromDisk(const CBlockIndex *pindex) {
    if (!ReadFromDisk(pindex->GetBlockPos())) return false;
...
bool CBlock::ReadFromDisk(const CDiskBlockPos &pos) {
    ...
    CAutoFile filein =
        CAutoFile(OpenBlockFile(pos, true), SER_DISK, CLIENT_VERSION);
    if (!filein) return error("CBlock::ReadFromDisk() : OpenBlockFile failed");
...
bool CBlockIndex::CheckIndex() const {
    if (pprev) {
        ...
        uint256 seedHash = Hash(BEGIN(tempHash), END(tempHash));
        return CheckProofOfWork(seedHash, nBits, pprev->GetBlockHash(),
                                nVersion, nNonce);
    } else {
        uint256 initHash = 0;
...
void static InvalidChainFound(CBlockIndex *pindexNew) {
    ...
    printf(
        "InvalidChainFound: invalid block=%s  height=%d  work=%llu  date=%s\n",
        pindexNew->GetBlockHash().ToString().c_str(), pindexNew->nHeight,
        pindexNew->nChainWork,
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexNew->GetBlockTime())
            .c_str());
    printf(
        "InvalidChainFound:  current best=%s  height=%d  work=%llu  date=%s\n",
        hashBestChain.ToString().c_str(), nBestHeight, nBestChainWork,
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexBest->GetBlockTime())
            .c_str());
    if (pindexBest)
        printf(
            "InvalidChainFound: Warning: Displayed transactions may not be "
...
bool ConnectBestBlock(CValidationState &state) {
    do {
...

        {
            ...
            if (it == setBlockIndexValid.rend()) return true;
            pindexNewBest = *it;
        }

        if (pindexNewBest == pindexBest ||
            (pindexBest && pindexNewBest->nChainWork == pindexBest->nChainWork))
        ...
        do {
            ...
            if (pindexTest->pprev == NULL || pindexTest->pnext != NULL) {
                ...
                BOOST_FOREACH (CBlockIndex *pindexSwitch, vAttach) {
                    ...
                    try {
                        if (!SetBestChain(state, pindexSwitch)) return false;
                    } catch (std::runtime_error &e) {
                        return state.Abort(_("System error: ") + e.what());
                    }
                }
...
void CBlockHeader::UpdateTime(const CBlockIndex *pindexPrev) {
    ...
    if (fTestNet) nBits = GetNextWorkRequired(pindexPrev, this);
}
...
bool CTransaction::HaveInputs(CCoinsViewCache &inputs) const {
    if (!IsCoinBase()) {
        ...
        for (unsigned int i = 0; i < vin.size(); i++) {
            ...
            if (!inputs.HaveCoins(prevout.hash)) return false;
        }
        ...
        for (unsigned int i = 0; i < vin.size(); i++) {
            ...
            if (!coins.IsAvailable(prevout.n)) return false;
        }
    }
    return true;
}
...
bool CScriptCheck::operator()() const {
    ...
    if (!VerifyScript(scriptSig, scriptPubKey, *ptxTo, nIn, nFlags, nHashType))
        return error("CScriptCheck() : %s VerifyScript failed",
                     ptxTo->GetHash().ToString().c_str());
    return true;
}

bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs,
                               bool fScriptChecks, unsigned int flags,
                               std::vector<CScriptCheck> *pvChecks) const {
    if (!IsCoinBase()) {
        if (pvChecks) pvChecks->reserve(vin.size());

        // This doesn't trigger the DoS code on purpose; if it did, it would
        // make it easier for an attacker to attempt to split the network.
        if (!HaveInputs(inputs))
            return state.Invalid(error("CheckInputs() : %s inputs unavailable",
        ...
        for (unsigned int i = 0; i < vin.size(); i++) {
            ...
            if (!MoneyRange(coins.vout[prevout.n].nValue) ||
                !MoneyRange(nValueIn))
                return state.DoS(
                    100, error("CheckInputs() : txin values out of range"));
        }
        ...
        if (nTxFee < 0)
            return state.DoS(100, error("CheckInputs() : %s nTxFee < 0",
                                        GetHash().ToString().c_str()));
        nFees += nTxFee;
        if (!MoneyRange(nFees))
        ...
        // before the last block chain checkpoint. This is safe because block
        // merkle hashes are still computed and checked, and any change will be
            ...
            for (unsigned int i = 0; i < vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & SCRIPT_VERIFY_STRICTENC) {
                        // For now, check whether the failure was caused by
                        // non-canonical encodings or not; if so, don't trigger
                        // DoS protection.
                        CScriptCheck check(coins, (CTransaction *)this, i,
                                           flags & (~SCRIPT_VERIFY_STRICTENC),
                                           0);
                        if (check()) return state.Invalid();
                    }
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex,
                             CCoinsViewCache &view, bool *pfClean) {
    ...
    for (int i = vtx.size() - 1; i >= 0; i--) {
        ...
        if (!view.HaveCoins(hash)) {
            fClean = fClean && error(
                                   "DisconnectBlock() : outputs still spent? "
                                   "database corrupted");
        ...
        if (outs != outsBlock)
            fClean = fClean && error(
                                   "DisconnectBlock() : added transaction "
        ...
        if (i > 0) {  // not coinbases
            ...
            if (txundo.vprevout.size() != tx.vin.size())
                return error(
                    "DisconnectBlock() : transaction and undo data "
                    "inconsistent");
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                const COutPoint &out = tx.vin[j].prevout;
                const CTxInUndo &undo = txundo.vprevout[j];
                CCoins coins;
                view.GetCoins(out.hash, coins);  // this can fail if the prevout
                                                 // was already entirely spent
                if (undo.nHeight != 0) {
                    // undo data contains height: this is the last output of the
                    // prevout tx being spent
                    if (!coins.IsPruned())
                        fClean =
                            fClean && error(
                                          "DisconnectBlock() : undo data "
                    ...
                    if (coins.IsPruned())
                        fClean = fClean &&
                                 error(
                                     "DisconnectBlock() : undo data adding "
                                     "output to missing transaction");
                }
...
void static FlushBlockFile(bool fFinalize = false) {
    ...
    if (fileOld) {
        if (fFinalize) TruncateFile(fileOld, infoLastBlockFile.nSize);
        FileCommit(fileOld);
        fclose(fileOld);
    }
    ...
    if (fileOld) {
        if (fFinalize) TruncateFile(fileOld, infoLastBlockFile.nUndoSize);
        FileCommit(fileOld);
        fclose(fileOld);
    }
}
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex *pindex,
    ...
    if (!CheckBlock(state, !fJustCheck, !fJustCheck)) return false;
    ...
    if (nVersion >= 3 && ((!fTestNet && CBlockIndex::IsSuperMajority(
                                            3, pindex->pprev, 750, 1000)) ||
                          (fTestNet && CBlockIndex::IsSuperMajority(
                                           3, pindex->pprev, 51, 100)))) {
    ...
    for (unsigned int i = 0; i < vtx.size(); i++) {
        ...
        if (!tx.IsCoinBase()) blockundo.vtxundo.push_back(txundo);
    ...
    if (fBenchmark)
        printf("- Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin)\n",
               (unsigned)vtx.size(), 0.001 * nTime, 0.001 * nTime / vtx.size(),
               nInputs <= 1 ? 0 : 0.001 * nTime / (nInputs - 1));

    if (vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees) &&
        GetHash() != hashGenesisBlock)
        return state.DoS(
            100,
            error("ConnectBlock() : coinbase pays too much (actual=%" PRI64d
                  " vs limit=%" PRI64d ")",
    ...
    if ((GetHash() != hashGenesisBlock) &&
        (pindex->GetUndoPos().IsNull() ||
         (pindex->nStatus & BLOCK_VALID_MASK) < BLOCK_VALID_SCRIPTS)) {
        if (pindex->GetUndoPos().IsNull()) {
            CDiskBlockPos pos;
            if (!FindUndoPos(
                    state, pindex->nFile, pos,
...
bool SetBestChain(CValidationState &state, CBlockIndex *pindexNew) {
    ...
    for (CBlockIndex *pindex = view.GetBestBlock(); pindex != pfork;
         pindex = pindex->pprev)
    ...
    BOOST_FOREACH (CBlockIndex *pindex, vDisconnect) {
        ...
        // We only do this for blocks after the last checkpoint (reorganisation
        // before that point should only happen with -reindex/-loadblock, or a
        // misbehaving peer.
    ...
    // Connect longer branch
    BOOST_FOREACH (CBlockIndex *pindex, vConnect)
        if (pindex->pprev) pindex->pprev->pnext = pindex;
    ...
    nTransactionsUpdated++;
    printf(
        "SetBestChain: new best=%s  height=%d  work=%llu  tx=%lu  date=%s "
        "progress=%f\n",
        hashBestChain.ToString().c_str(), nBestHeight, nBestChainWork,
    ...
    if (!fIsInitialDownload) {
        ...
        for (int i = 0; i < 100 && pindex != NULL; i++) {
            if (pindex->nVersion > BLOCK_CURRENT_VERSION) ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
bool CBlock::AddToBlockIndex(CValidationState &state,
    ...
    if (mapBlockIndex.count(hash))
        return state.Invalid(error("AddToBlockIndex() : %s already exists",
                                   hash.ToString().c_str()));
    ...
    pindexNew->nChainTx =
        (pindexNew->pprev ? pindexNew->pprev->nChainTx : 0) + pindexNew->nTx;
    ...
    // New best?
    if (!ConnectBestBlock(state)) return false;
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos,
                  unsigned int nAddSize, unsigned int nHeight, uint64 nTime,
                  bool fKnown = false) {
    ...
    if (fKnown) {
        ...
        while (infoLastBlockFile.nSize + nAddSize >= MAX_BLOCKFILE_SIZE) {
            ...
            pblocktree->ReadBlockFileInfo(
                nLastBlockFile,
                infoLastBlockFile);  // check whether data for the new file
                                     // somehow already exist; can fail just
                                     // fine
            fUpdatedLast = true;
        }
    ...
    if (!fKnown) {
        ...
        unsigned int nNewChunks =
            (infoLastBlockFile.nSize + BLOCKFILE_CHUNK_SIZE - 1) /
        ...
        if (nNewChunks > nOldChunks) {
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    printf(
                        "Pre-allocating up to position 0x%x in blk%08u.dat\n",
    ...
    if (fUpdatedLast) pblocktree->WriteLastBlockFile(nLastBlockFile);
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW,
                        bool fCheckMerkleRoot) const {
    ...
    uint256 seedHash = Hash(BEGIN(tempHash), END(tempHash));
    if (fCheckPOW &&
        !CheckProofOfWork(seedHash, nBits, hashPrevBlock, nVersion, nNonce))
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp) {
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Reject block.nVersion=1 blocks when 95% (75% on testnet) of the
        // network has upgraded:
        if (nVersion < 2) {
            if ((!fTestNet &&
                 CBlockIndex::IsSuperMajority(2, pindexPrev, 950, 1000)) ||
                (fTestNet &&
                 CBlockIndex::IsSuperMajority(2, pindexPrev, 75, 100))) {
                return state.Invalid(
                    error("AcceptBlock() : rejected nVersion=1 block"));
            }
        }
        // Reject block.nVersion=2 blocks when 95% (75% on testnet) of the
        // network has upgraded:
        if (nVersion < 3) {
            if ((!fTestNet &&
                 CBlockIndex::IsSuperMajority(3, pindexPrev, 950, 1000)) ||
                (fTestNet &&
                 CBlockIndex::IsSuperMajority(3, pindexPrev, 75, 100))) {
                return state.Invalid(
                    error("AcceptBlock() : rejected nVersion=2 block"));
            }
        }
        // Enforce block.nVersion=2 rule that the coinbase starts with
        // serialized block height
        if (nVersion >= 2) {
            // if 750 of the last 1,000 blocks are version 2 or greater (51/100
            // if testnet):
            if ((!fTestNet &&
                 CBlockIndex::IsSuperMajority(2, pindexPrev, 750, 1000)) ||
                (fTestNet &&
                 CBlockIndex::IsSuperMajority(2, pindexPrev, 51, 100))) {
                CScript expect = CScript() << nHeight;
                if (vtx[0].vin[0].scriptSig.size() < expect.size() ||
                    !std::equal(expect.begin(), expect.end(),
                                vtx[0].vin[0].scriptSig.begin()))
                    return state.DoS(100, error("AcceptBlock() : block height "
                                                "mismatch in coinbase"));
            }
        }
    }
...
bool ProcessBlock(CValidationState &state, CNode *pfrom, CBlock *pblock,
    ...
    if (mapBlockIndex.count(hash))
        return state.Invalid(error("ProcessBlock() : already have block %d %s",
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain) {
        ...
        if (deltaTime < 0) {
            return state.DoS(100, error("ProcessBlock() : block with timestamp "
    ...
    // If we don't already have its previous block, shunt it off to holding area
    // until we get it
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
        ...
        for (multimap<uint256, CBlock *>::iterator mi =
            ...
            // counter-DoS based on orphan resolution (that is, feeding people
            // an invalid block based on LegitBlockX in order to get anyone
...
uint256 CPartialMerkleTree::CalcHash(int height, unsigned int pos,
    ...
    if (height == 0) {
        ...
        // calculate right hash if not beyong the end of the array - copy left
        // hash otherwise1
...
uint256 CPartialMerkleTree::TraverseAndExtract(int height, unsigned int pos,
    ...
    if (height == 0 || !fParentOfMatch) {
        ...
        if (height == 0 &&
            fParentOfMatch)  // in case of height 0, we have a matched txid
...
uint256 CPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch) {
    ...
    if (nTransactions >
        MAX_BLOCK_SIZE / 60)  // 60 is the lower bound for the size of a
                              // serialized CTransaction
        return 0;
    // there can never be more hashes provided than one for every txid
    if (vHash.size() > nTransactions) return 0;
    // there must be at least one bit per node in the partial tree, and at least
    // one node per hash
    if (vBits.size() < vHash.size()) return 0;
    // calculate height of tree
    int nHeight = 0;
    while (CalcTreeWidth(nHeight) > 1) nHeight++;
    // traverse the partial tree
    ...
    uint256 hashMerkleRoot =
        TraverseAndExtract(nHeight, 0, nBitsUsed, nHashUsed, vMatch);
    // verify that no problems occured during the tree traversal
    if (fBad) return 0;
    // verify that all bits were consumed (except for the padding caused by
    // serializing it as a byte sequence)
    if ((nBitsUsed + 7) / 8 != (vBits.size() + 7) / 8) return 0;
    // verify that all hashes were consumed
    if (nHashUsed != vHash.size()) return 0;
...
FILE *OpenDiskFile(const CDiskBlockPos &pos, const char *prefix,
    ...
    boost::filesystem::path path =
        GetDataDir() / "blocks" / strprintf("%s%08u.dat", prefix, pos.nFile);
...
bool VerifyDB() {
    if (pindexBest == NULL || pindexBest->pprev == NULL) return true;
    ...
    if (nCheckDepth > nBestHeight) nCheckDepth = nBestHeight;
    ...
    for (CBlockIndex *pindex = pindexBest; pindex && pindex->pprev;
         pindex = pindex->pprev) {
        boost::this_thread::interruption_point();
        if (pindex->nHeight < nBestHeight - nCheckDepth) break;
        CBlock block;
        ...
        if (nCheckLevel >= 3 && pindex == pindexState &&
            (coins.GetCacheSize() + pcoinsTip->GetCacheSize()) <=
                2 * nCoinCacheSize + 320000) {
            bool fClean = true;
            if (!block.DisconnectBlock(state, pindex, coins, &fClean))
                return error(
                    "VerifyDB() : *** irrecoverable inconsistency in block "
                    "data at %d, hash=%s",
                    pindex->nHeight, pindex->GetBlockHash().ToString().c_str());
    ...
    if (pindexFailure)
        return error(
            "VerifyDB() : *** coin database inconsistencies found (last %i "
...
bool LoadBlockIndex() {
    if (fTestNet) {
        ...
        hashGenesisBlock = uint256(
            "0xa746824905c4039b034e2103537d2c879a7543e8c1c0931367245619d36d4e1"
            "f");
    }
    ...
    //
    if (!fReindex && !LoadBlockIndexDB()) return false;
...
bool InitBlockIndex() {
    ...
    if (pindexGenesisBlock != NULL) return true;
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey.SetDestination(
            CKeyID("e6b765cf4efeb5519bcb47ca35738355588e44284398d58c4ac33116ab7"
                   "e8cef"));
        txNew.vout[1].nValue = 20000000 * COIN;
        txNew.vout[1].scriptPubKey.SetDestination(
            CKeyID("ebbb5a58e2914878d6f6670a53bdd5e6d0210a3d0b3fa8656bfeb8dd61f"
                   "b8b81"));
        txNew.vout[2].nValue = 20000000 * COIN;
        txNew.vout[2].scriptPubKey.SetDestination(
            CKeyID("76068b3357435c9306fcb2b54547b298b5f19e4819dd62e31f84558219e"
                   "3bd8e"));
        txNew.vout[3].nValue = 20000000 * COIN;
        txNew.vout[3].scriptPubKey.SetDestination(
            CKeyID("be17335dbc8aeaf6aeb3c4ae62886491dab4a3c38c79a5e86bac95214a4"
                   "9b001"));
        txNew.vout[4].nValue = 20000000 * COIN;
        txNew.vout[4].scriptPubKey.SetDestination(
            CKeyID("f6b7bee668d7125d8a5502ccf9f5927b8866dc180d20642e368d9a936e2"
                   "87e39"));
        txNew.vout[5].nValue = 20000000 * COIN;
        txNew.vout[5].scriptPubKey.SetDestination(
            CKeyID("706d4053c3794cc3312a7c09b6dd21dd1da3f8c8e56f23c76554ddbf585"
                   "bccf4"));
        txNew.vout[6].nValue = 20000000 * COIN;
        txNew.vout[6].scriptPubKey.SetDestination(
            CKeyID("bdb56bbe25f5db3df8a3dbfbd5480f390665603b0bc1f6120d0a36370d4"
                   "d5416"));
        txNew.vout[7].nValue = 20000000 * COIN;
        txNew.vout[7].scriptPubKey.SetDestination(
            CKeyID("8603abbecfdcaa59545ba185caf684a75075d4147afba303b2c318ef58f"
                   "6c33e"));
        txNew.vout[8].nValue = 20000000 * COIN;
        txNew.vout[8].scriptPubKey.SetDestination(
            CKeyID("ebe47a2e627e8c015534830474268becf1ea37da47c8a426b2336ed72ab"
                   "c3257"));
        txNew.vout[9].nValue = 20000000 * COIN;
        txNew.vout[9].scriptPubKey.SetDestination(
            CKeyID("ec97680cfe5c7f30b6356329d463b2e00ed09c22a37ca8010a9c84e36b7"
                   "0f674"));
        txNew.vout[10].nValue = 7374182.7 * COIN;
        txNew.vout[10].scriptPubKey.SetDestination(
            CKeyID("210ed7d715c525c5bf6b638c29428c70bc4040f248750d5241371d00d84"
                   "f22ef"));
        txNew.vout[11].nValue = 7374182 * COIN;
        txNew.vout[11].scriptPubKey.SetDestination(
            CKeyID("b4cb22b62f91f6a36554bdfce5b860d6905c54217fd4543d8eaf16bb20d"
                   "6ccf7"));
        ...
        block.nNonce = uint256(
            "0x0000000000000000000000000000000000000000000000000001ee7340a9a1d"
            "6");

        if (fTestNet) {
            block.nTime = 1529291280;
            block.nNonce = uint256(
                "0x0000000000000000000000000000000000000000000000000001ee7340a9"
                "a1d6");
        }
        ...
        assert(block.hashMerkleRoot ==
               uint256("0xaef6a6cb3767fa5d965b10f9d1e3e183ddea21a5f7ffce9bd7a86"
                       "c065e7c6865"));
        block.print();
...
void PrintBlockTree() {
    ...
    while (!vStack.empty()) {
        ...
        printf("%d (blk%08u.dat:0x%x)  %s  tx %" PRIszu "", pindex->nHeight,
    ...
    try {
        ...
        while (blkdat.good() && !blkdat.eof()) {
            ...
            try {
                ...
                blkdat >> FLATDATA(buf);
                if (memcmp(buf, pchMessageStart, 4)) continue;
                // read size
                blkdat >> nSize;
                if (nSize < 80 || nSize > MAX_BLOCK_SIZE) continue;
            } catch (std::exception &e) {
            ...
            try {
                ...
                if (nBlockPos >= nStartByte) {
                    ...
                    CValidationState state;
                    if (ProcessBlock(state, NULL, &block, dbp)) nLoaded++;
...
string GetWarnings(string strFor) {
    ...
    if (GetBoolArg("-testsafemode")) strRPC = "test";
    ...
    if (pindexBest &&
        nBestInvalidWork > nBestChainWork + pindexBest->GetBlockWork()) {
        nPriority = 2000;
        strStatusBar = strRPC =
            _("Warning: Displayed transactions may not be correct! You may "
              "need to upgrade, or other nodes may need to upgrade.");
    }
...
// - 1) + 9 (this is decodable, as d is in [1-9] and e is in [0-9])
...
class CMainCleanup {
   public:
    ...
    ~CMainCleanup() {
        ...
        for (; it2 != mapOrphanBlocks.end(); it2++) delete (*it2).second;
