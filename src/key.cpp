...
void CKey::Reset() { privKey.resize(RAINBOW_PRIVATE_KEY_SIZE); }
...
bool CPubKey::Verify(uint256 hash, const std::vector<unsigned char>& vchSig) {
    ...
    int status = -1;
    status =
        rainbow_verify((unsigned char*)&hash, &vchSig[0], vchPubKey.data());
    if (status != 0) {
        return false;
    }
    return true;
...
bool CKey::SetPubKey(std::vector<unsigned char> pk) {
    if (pk.size() != RAINBOW_PUBLIC_KEY_SIZE) return false;
    pubKey.vchPubKey.resize(RAINBOW_PUBLIC_KEY_SIZE);
