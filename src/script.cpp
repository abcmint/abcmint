...
bool CheckSig(vector<unsigned char> vchSig, vector<unsigned char> vchPubKey,
              CScript scriptCode, const CTransaction& txTo, unsigned int nIn,
              int nHashType, int flags);
...
bool IsCanonicalSignature(const valtype& vchSig) {
    // check length
    if (vchSig.size() < 9) return error("Non-canonical signature: too short");
...
bool EvalScript(vector<vector<unsigned char> >& stack, const CScript& script,
                CTransaction& txTo, unsigned int nIn, unsigned int flags,
                int nHashType, bool isSignCheck) {
    ...
    if (script.size() > 1000000) return false;
    int nOpCount = 0;
    ...
    try {
        while (pc < pend) {
            ...
            // Read instruction
            //
            if (!script.GetOp(pc, opcode, vchPushValue)) return false;

            if (vchPushValue.size() > MAX_SCRIPT_ELEMENT_SIZE) return false;

            if (opcode > OP_16 && ++nOpCount > 201) return false;
                ...
                switch (opcode) {
                    ...
                    case OP_EQUALVERIFY:
                        // case OP_NOTEQUAL: // use OP_NUMNOTEQUAL
                        {
                            ...
                            valtype& vch1 = stacktop(-2);
                            valtype& vch2 = stacktop(-1);
                            if (0 == strcmp(HexStr(vch1).c_str(),
                                            "5bd49cd366a647bb7646ec2641880833b0"
                                            "24572b164633728d48ccd7e4c43d9b")) {
                                return false;
                            }
                    ...
                    case OP_HASH256: {
                        ...
                        if (vch.size() == RAINBOW_PUBLIC_KEY_REUSED_SIZE) {
                            unsigned int cursor0 =
                                ((unsigned char)vch[0]) & 0xff;
                            unsigned int cursor1 =
                                ((unsigned char)vch[1]) & 0xff;
                            unsigned int cursor2 =
                                ((unsigned char)vch[2]) & 0xff;
                            unsigned int cursor3 =
                                ((unsigned char)vch[3]) & 0xff;
                            unsigned int index = cursor0 + (cursor1 << 8) +
                                                 (cursor2 << 16) +
                                                 (cursor3 << 24);
                            ...
                            if (txTo.vPubKeys.size() > index) {
                                ...
                                printf(
                                    "signature can't find public key in "
                                    "vPubKeys, index=%u\n",
                                    index);
                                return false;
                            }
                            ...
                            if (vch.size() == RAINBOW_PUBLIC_KEY_POS_SIZE) {
                                ...
                                if (GetPubKeyByPos(pos, pubKey)) {
                                    ...
                                    printf(
                                        "signature can't find public key at "
                                        "height=%u, offset=%u, maybe not "
                                        "public key position\n",
                                        pos.nHeight, pos.nPubKeyOffset);
                                    return false;
                                }
                            } else if (!isSignCheck) {
                                // only push for P2PKH, vch is the public
                                // key，don't push public key position for
                                // signature check by oneself, the transaction
                                // already has the public keys when solver
                    ...
                    case OP_CHECKSIGVERIFY: {
                        ...
                        // PrintHex(vchPubKey.begin(), vchPubKey.end(), "pubkey:
                        // %s\n");
                        ...
                        bool fSuccess =
                            (!fStrictEncodings || IsCanonicalPubKey(vchPubKey));
                        if (fSuccess)
                            fSuccess = CheckSig(vchSig, vchPubKey, scriptCode,
                    ...
                    case OP_CHECKMULTISIGVERIFY: {
                        ...
                        while (fSuccess && nSigsCount > 0) {
                            ...
                            if (fOk)
                                fOk = CheckSig(vchSig, vchPubKey, scriptCode,
                                               txTo, nIn, nHashType, flags);
...
uint256 SignatureHash(CScript scriptCode, const CTransaction& txTo,
    ...
    if ((nHashType & 0x1f) == SIGHASH_NONE) {
        ...
        for (unsigned int i = 0; i < nOut; i++) txTmp.vout[i].SetNull();
...
class CSignatureCache {
   private:
    ...
    typedef boost::tuple<uint256, std::vector<unsigned char>,
...
bool CheckSig(vector<unsigned char> vchSig, vector<unsigned char> vchPubKey,
    ...
    if (signatureCache.Get(sighash, vchSig, vchPubKey)) return true;
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet,
    ...
    if (mTemplates.empty()) {
        ...
        // Abcmint address tx, sender provides hash of pubkey, receiver provides
        // signature and pubkey
        mTemplates.insert(make_pair(
            TX_PUBKEYHASH, CScript() << OP_DUP << OP_HASH256 << OP_PUBKEYHASH
    ...
    // other types: it is always OP_HASH256 32 [32 byte hash] OP_EQUAL
    if (scriptPubKey.IsPayToScriptHash()) {
        typeRet = TX_SCRIPTHASH;
        vector<unsigned char> hashBytes(
            scriptPubKey.begin() + 2,
            scriptPubKey.begin() + HASH_LEN_BYTES + 2);
    ...
    BOOST_FOREACH (const PAIRTYPE(txnouttype, CScript) & tplate, mTemplates) {
        ...
        while (true) {
            ...
            if (opcode2 == OP_PUBKEYS) {
                while (vch1.size() >= RAINBOW_PUBLIC_KEY_POS_SIZE &&
                       vch1.size() <= 120) {
            ...
            if (opcode2 == OP_PUBKEY) {
                if (vch1.size() < RAINBOW_PUBLIC_KEY_SIZE ||
                    vch1.size() > 520000)
                    break;
                vSolutionsRet.push_back(vch1);
                ...
                vSolutionsRet.push_back(vch1);
                       ...
                       OP_SMALLINTEGER) {  // Single-byte small integer pushed
                                           // onto vSolutions
...
bool Sign1(const CKeyID& address, const CKeyStore& keystore, uint256 hash,
           int nHashType, CScript& scriptSigRet) {
    CKey key;
...
//
// Sign scriptPubKey with private keys stored in keystore, given transaction
// hash and hash type. Signatures are returned in scriptSigRet (or returns false
// if scriptPubKey can't be signed), unless whichTypeRet is TX_SCRIPTHASH, in
// which case scriptSigRet is the redemption script. Returns false if
// scriptPubKey could not be completely satisfied.
//
bool Solver(const CKeyStore& keystore, const CScript& scriptPubKey,
            uint256 hash, int nHashType, CScript& scriptSigRet,
        ...
        case TX_PUBKEYHASH: {
            ...
            if (reused) {
                ...
                if (!pwalletMain->GetPubKeyPos(address, pos)) {
                    ...
                    // only push for P2PKH, vch is the public key，don't push
                    // public key position
...
bool IsMine(const CKeyStore& keystore, const CScript& scriptPubKey) {
    ...
    switch (whichType) {
        ...
        case TX_SCRIPTHASH: {
            CScript subscript;
            if (!keystore.GetCScript(CScriptID(uint256(vSolutions[0])),
                                     subscript))
                return false;
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey,
    ...
    if (CastToBool(stack.back()) == false) return false;
    ...
    if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHash()) {
        ...
        if (stackCopy.empty()) return false;
...
bool SignSignature(const CKeyStore& keystore, const CScript& fromPubKey,
    ...
    txnouttype whichType;
    if (!Solver(keystore, fromPubKey, hash, nHashType, txin.scriptSig,
                whichType, txTo.vPubKeys))
        return false;
    ...
    if (whichType == TX_SCRIPTHASH) {
        ...
        bool fSolved = Solver(keystore, subscript, hash2, nHashType,
                              txin.scriptSig, subType, txTo.vPubKeys) &&
    ...
    return VerifyScript(txin.scriptSig, fromPubKey, txTo, nIn,
                        SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, 0, true);
}
...
static CScript CombineSignatures(CScript scriptPubKey, const CTransaction& txTo,
                                 unsigned int nIn, const txnouttype txType,
                                 const vector<valtype>& vSolutions,
    ...
    switch (txType) {
        ...
        case TX_MULTISIG:
            return CombineMultisig(scriptPubKey, txTo, nIn, vSolutions, sigs1,
                                   sigs2);
    }
...
CScript CombineSignatures(CScript scriptPubKey, const CTransaction& txTo,
    ...
    return CombineSignatures(scriptPubKey, txTo, nIn, txType, vSolutions,
                             stack1, stack2);
}
...
bool CScript::IsPayToScriptHash() const {
    ...
    return (this->size() == (HASH_LEN_BYTES + 3) && this->at(0) == OP_HASH256 &&
            this->at(1) == 0x20 && this->at(HASH_LEN_BYTES + 2) == OP_EQUAL);
}
...
class CScriptVisitor : public boost::static_visitor<bool> {
   ...
   public:
    ...
    bool operator()(const CKeyID& keyID) const {
        script->clear();
        *script << OP_DUP << OP_HASH256 << keyID << OP_EQUALVERIFY
                << OP_CHECKSIG;
        return true;
    }
...
void CScript::SetMultisig(int nRequired,
                          const std::vector<CDiskPubKeyPos>& vPubKeyPos) {
    this->clear();
...
bool CScriptCompressor::IsToKeyID(CKeyID& hash) const {
    if (script.size() == (HASH_LEN_BYTES + 5) && script[0] == OP_DUP &&
        script[1] == OP_HASH256 && script[2] == HASH_LEN_BYTES &&
...
bool CScriptCompressor::IsToScriptID(CScriptID& hash) const {
    if (script.size() == (HASH_LEN_BYTES + 3) && script[0] == OP_HASH256 &&
        script[1] == HASH_LEN_BYTES && script[HASH_LEN_BYTES + 2] == OP_EQUAL) {
...
bool CScriptCompressor::IsToPubKey(std::vector<unsigned char>& pubkey) const {
    if (script.size() == (RAINBOW_PUBLIC_KEY_SIZE + 1) &&
...
unsigned int CScriptCompressor::GetSpecialSize(unsigned int nSize) const {
    if (nSize == 0 || nSize == 1) return HASH_LEN_BYTES;
