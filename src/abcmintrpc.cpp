...
void RPCTypeCheck(const Array& params, const list<Value_type>& typesExpected,
    ...
    BOOST_FOREACH (Value_type t, typesExpected) {
        ...
        if (!((v.type() == t) || (fAllowNull && (v.type() == null_type)))) {
            string err =
                strprintf("Expected type %s, got %s", Value_type_name[t],
    ...
    BOOST_FOREACH (const PAIRTYPE(string, Value_type) & t, typesExpected) {
        ...
        if (!fAllowNull && v.type() == null_type)
            throw JSONRPCError(RPC_TYPE_ERROR,
                               strprintf("Missing %s", t.first.c_str()));

        if (!((v.type() == t.second) ||
              (fAllowNull && (v.type() == null_type)))) {
            string err = strprintf("Expected type %s for %s, got %s",
                                   Value_type_name[t.second], t.first.c_str(),
                                   Value_type_name[v.type()]);
            throw JSONRPCError(RPC_TYPE_ERROR, err);
        }
    }
...
Value stop(const Array& params, bool fHelp) {
    ...
    StartShutdown();
    return "Abcmint server stopping";
}
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive) {
    ...
    return strprintf(
        ...
        "\r\n"
        "%s",
        nStatus, cStatus, rfc1123Time().c_str(),
        keepalive ? "keep-alive" : "close", strMsg.size(),
        FormatFullVersion().c_str(), strMsg.c_str());
}
...
bool ReadHTTPRequestLine(std::basic_istream<char>& stream, int& proto,
    ...
    if (vWords.size() < 2) return false;

    // HTTP methods permitted: GET, POST
    http_method = vWords[0];
    if (http_method != "GET" && http_method != "POST") return false;

    // HTTP URI must be an absolute path, relative to current host
    http_uri = vWords[1];
    if (http_uri.size() == 0 || http_uri[0] != '/') return false;

    // parse proto, if present
    string strProto = "";
    if (vWords.size() > 2) strProto = vWords[2];

    proto = 0;
    const char* ver = strstr(strProto.c_str(), "HTTP/1.");
    if (ver != NULL) proto = atoi(ver + 7);
...
int ReadHTTPStatus(std::basic_istream<char>& stream, int& proto) {
    ...
    vector<string> vWords;
    boost::split(vWords, str, boost::is_any_of(" "));
    if (vWords.size() < 2) return HTTP_INTERNAL_SERVER_ERROR;
    proto = 0;
...
int ReadHTTPHeaders(std::basic_istream<char>& stream,
    ...
    while (true) {
        ...
        if (nColon != string::npos) {
            ...
            if (strHeader == "content-length") nLen = atoi(strValue.c_str());
        }
    }
    return nLen;
}
...
int ReadHTTPMessage(std::basic_istream<char>& stream,
                    map<string, string>& mapHeadersRet, string& strMessageRet,
                    int nProto) {
    mapHeadersRet.clear();
    strMessageRet = "";
    ...
    if (nLen < 0 || nLen > (int)MAX_SIZE) return HTTP_INTERNAL_SERVER_ERROR;
...
//
// JSON-RPC protocol.  Abcmint speaks version 1.0 for maximum compatibility,
...
string JSONRPCRequest(const string& strMethod, const Array& params,
                      const Value& id) {
    Object request;
...
Object JSONRPCReplyObj(const Value& result, const Value& error,
                       const Value& id) {
    Object reply;
...
bool ClientAllowed(const boost::asio::ip::address& address) {
    // Make sure that IPv4-compatible and IPv4-mapped IPv6 addresses are treated
    // as IPv4 addresses
    if (address.is_v6() &&
        (address.to_v6().is_v4_compatible() || address.to_v6().is_v4_mapped()))
    ...
    BOOST_FOREACH (string strAllow, vAllow)
        if (WildcardMatch(strAddress, strAllow)) return true;
    return false;
}
...
class SSLIOStreamDevice : public iostreams::device<iostreams::bidirectional> {
   public:
    SSLIOStreamDevice(asio::ssl::stream<typename Protocol::socket>& streamIn,
                      bool fUseSSLIn)
        : stream(streamIn) {
        fUseSSL = fUseSSLIn;
...
class AcceptedConnectionImpl : public AcceptedConnection {
   public:
    AcceptedConnectionImpl(asio::io_service& io_service, ssl::context& context,
                           bool fUseSSL)
...
static void RPCAcceptHandler(
    boost::shared_ptr<basic_socket_acceptor<Protocol, SocketAcceptorService> >
        acceptor,
...
static void RPCListen(
    ...
    AcceptedConnectionImpl<Protocol>* conn =
        new AcceptedConnectionImpl<Protocol>(acceptor->get_io_service(),
                                             context, fUseSSL);

    acceptor->async_accept(
        conn->sslStream.lowest_layer(), conn->peer,
        boost::bind(&RPCAcceptHandler<Protocol, SocketAcceptorService>,
                    acceptor, boost::ref(context), fUseSSL, conn,
...
static void RPCAcceptHandler(
    ...
    const boost::system::error_code& error) {
    // Immediately start accepting new connections, except when we're cancelled
    // or our socket is closed.
    if (error != asio::error::operation_aborted && acceptor->is_open())
        RPCListen(acceptor, context, fUseSSL);

    AcceptedConnectionImpl<ip::tcp>* tcp_conn =
        dynamic_cast<AcceptedConnectionImpl<ip::tcp>*>(conn);
    ...
    // certain DoS and misbehaving clients.
        ...
        // Only send a 403 if we're not using SSL to prevent a DoS during the
        // SSL handshake.
        if (!fUseSSL)
...
void StartRPCThreads() {
    ...
    if ((mapArgs["-rpcpassword"] == "") ||
        ...
        string strWhatAmI = "To use abcmint";
        ...
        uiInterface.ThreadSafeMessageBox(
            strprintf(
                _("%s, you must set a rpcpassword in the configuration file:\n"
                  ...
                  "owner-readable-only file permissions.\n"
                  "It is also recommended to set alertnotify so you are "
                  "notified of problems;\n"
                  "for example: alertnotify=echo %%s | mail -s \"Abcmint "
                  "Alert\" admin@foo.com\n"),
    ...
    if (fUseSSL) {
        ...
        filesystem::path pathPKFile(
            GetArg("-rpcsslprivatekeyfile", "server.pem"));
        if (!pathPKFile.is_complete())
            pathPKFile = filesystem::path(GetDataDir()) / pathPKFile;
        if (filesystem::exists(pathPKFile))
            rpc_ssl_context->use_private_key_file(pathPKFile.string(),
                                                  ssl::context::pem);
        else
            printf(
        ...
        string strCiphers =
            GetArg("-rpcsslciphers",
                   "TLSv1+HIGH:!SSLv2:!aNULL:!eNULL:!AH:!3DES:@STRENGTH");
        SSL_CTX_set_cipher_list(rpc_ssl_context->impl(), strCiphers.c_str());
    }

    // Try a dual IPv6/IPv4 socket, falling back to separate IPv4 and IPv6
    // sockets
    const bool loopback = !mapArgs.count("-rpcallowip");
    asio::ip::address bindAddress = loopback ? asio::ip::address_v6::loopback()
                                             : asio::ip::address_v6::any();
    ip::tcp::endpoint endpoint(bindAddress,
                               GetArg("-rpcport", GetDefaultRPCPort()));
    ...
    try {
        ...
        // Try making the socket dual IPv6/IPv4 (if listening on the "any"
        // address)
        ...
        strerr =
            strprintf(_("An error occurred while setting up the RPC port %u "
                        "for listening on IPv6, falling back to IPv4: %s"),
                      endpoint.port(), e.what());
    }

    try {
        // If dual IPv6/IPv4 failed (or we're opening loopback interfaces only),
        // open IPv4 separately
        if (!fListening || loopback || v6_only_error) {
            bindAddress = loopback ? asio::ip::address_v4::loopback()
        ...
        strerr = strprintf(_("An error occurred while setting up the RPC port "
                             "%u for listening on IPv4: %s"),
                           endpoint.port(), e.what());
    }

    if (!fListening) {
        uiInterface.ThreadSafeMessageBox(strerr, "",
                                         CClientUIInterface::MSG_ERROR);
        StartShutdown();
        return;
    }
    ...
    for (int i = 0; i < GetArg("-rpcthreads", 4); i++)
        rpc_worker_group->create_thread(
...
static Object JSONRPCExecOne(const Value& req) {
    ...
    try {
        ...
        rpc_result = JSONRPCReplyObj(
            Value::null, JSONRPCError(RPC_PARSE_ERROR, e.what()), jreq.id);
    }
...
void ServiceConnection(AcceptedConnection* conn) {
    ...
    while (fRun) {
        ...
        if (strURI != "/") {
            conn->stream() << HTTPReply(HTTP_NOT_FOUND, "", false)
                           << std::flush;
            break;
        }
        ...
        if (mapHeaders.count("authorization") == 0) {
            conn->stream() << HTTPReply(HTTP_UNAUTHORIZED, "", false)
                           << std::flush;
            break;
        }
        ...
        if (mapHeaders["connection"] == "close") fRun = false;
        ...
        try {
            ...
            if (valRequest.type() == obj_type) {
                ...
                throw JSONRPCError(RPC_PARSE_ERROR,
            ...
            ErrorReply(conn->stream(), JSONRPCError(RPC_PARSE_ERROR, e.what()),
                       jreq.id);
            break;
        }
    }
...
Object CallRPC(const string& strMethod, const Array& params) {
    if (mapArgs["-rpcuser"] == "" && mapArgs["-rpcpassword"] == "")
        throw runtime_error(
            strprintf(_("You must set rpcpassword=<password> in the "
                        "configuration file:\n%s\n"
                        "If the file does not exist, create it with "
                        "owner-readable-only file permissions."),
    ...
    string strUserPass64 =
        EncodeBase64(mapArgs["-rpcuser"] + ":" + mapArgs["-rpcpassword"]);
...
Array RPCConvertValues(const std::string& strMethod,
    ...
    if (strMethod == "createmultisig" && n > 0)
        ConvertTo<boost::int64_t>(params[0]);
